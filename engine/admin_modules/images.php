...
// Файл: images.php
//-----------------------------------------------------------------------------/
// Назначение: Модуль управления картинками (админка)
//=============================================================================/
*/
	...
	if ($USER_RIGHTS['id_ug']==1) {
		...
		if ($a=="add" && $CONFIG['allow_dimg']) {
	        ...
	        if (!isset($action)) {
				...
				for ($i = 0; $i < $o_vars->post['kol_image']; $i++) {
					...
					if ($_FILES['image'.$i.'']['tmp_name']!="" && isset($_FILES['image'.$i.'']['tmp_name'])) {
	    ...
	    elseif ($a=="add" && !$CONFIG['allow_dimg']) $MESS1 = $o_other->showMessA($LANG['eaccess'], 1);
	    
	    if ((!isset($a) && !isset($i)) || ($a=="del" && isset($i)) || ($a=="add" && $sw)) {
	    	
	    	if ($a=="del") {
				
				$result = $o_images->del($i); 
	    	...
	    	if (isset($action) && $a!="add") {
					
				$make     = $o_vars->post['make'];
				$selected = $o_vars->post['selected'];
				
				$result = $o_images->makeIn($make, $selected);
	        ...
	        if (!$result) $o_tpl->addhtml("<div class=\"nothing\">".$LANG[$m.'_egets']."</div>");
	        	...
	        	foreach ($result as $n) {
					...
					if ($img_size[1] > 110) $mrules["{I_HEIGHT}"] = 110; else $mrules["{I_HEIGHT}"] = $img_size[1]; 
					if ($img_size[0] > 110) $mrules["{I_WIDTH}"]  = 110; else $mrules["{I_WIDTH}"]  = $img_size[0]; 
					
					$mrules["{FULL_NAME}"] = $n['name'];
						
					if (strlen($n['name']) > 13) {
						$a = explode(".",$n['name']);
						$mrules["{NAME}"] = substr($a[0], 0, 6)."... .". $a[1];
					}
	            	...
	            	$i++;
					if (!($i%6) && $i!=$k) $o_tpl->addhtml("</div><div class=\"conim\">");		
