...
// Файл: arhnews.php
//-----------------------------------------------------------------------------/
// Назначение: Модуль вывода архива новостей
//=============================================================================/
*/
	...
	$p    = $o_vars->get['p'];
	$year = $o_vars->get['year'];
	$mon  = $o_vars->get['mon'];
	$day  = ($o_vars->get['day']) ? $o_vars->get['day'] : 0;
	...
	$kol_news_on_p = $CONFIG['news_num'];
 	...
 	if (!$year || !$mon) {
 		...
 		if (!$result) $o_other->showMess($LANG[$m], $LANG[$m.'_egets'], "index.php", $LANG['redir_main'], TEMPLATES_DIR); 
			...
			$mrules["{MIDDLE}"] = "";
			foreach($result as $n) 
				$mrules1["{MIDDLE}"] .= "<a href=\"index.php?m=arhnews&amp;year=".$n['year']."&mon=".$n['mon']."\">".$n['year']." ".$o_other->getMonthAb($n['mon'])." (".$n['kol'].")</a><br />"; 
	 	...
	 	$result = $o_news->getS($p, $kol_news_on_p, $year, $mon, $day, false, $CONFIG['news_sort'], $CONFIG['news_msort']);
		if (!$result) $o_other->showMess($LANG[$m], $LANG['news_egets'], "index.php", $LANG['redir_main'], TEMPLATES_DIR);
		else {   
		    $kol_news = $o_news->kolNewsS($year, $mon, $day, $h);
	   	    $banners_top    = "";
	        $banners_middle = "";
	        $banners_buttom = "";
	        $result1 = $o_banners->getS(2);
	        if ($result1) {
		...
		if ($result) {
			$kol_p = ceil($kol_news / $kol_news_on_p);		    if ($kol_p > 1) {
        		...
        		if ($p > 0) $mrules["{PRIV}"] = "<a href=\"index.php?m=$m&amp;year=$year&amp;mon=$mon$day&amp;p=".($p-1)."\">".$LANG['spriv']."</a>";
			    ...
			    for ($i = 0, $t1 = 1, $t2 = 1; $i < $kol_p; $i++) {
				    if ($kol_p > 13) {   
				        ...
				        if ($i < 4 || ($i-$p) < 2 && $p < 6) $mrules["{LPAGES}"] .= ($i==$p) ? "<span class=\"sp\">".($i+1)."</span> " : "<a href=\"index.php?m=$m&amp;year=$year&amp;mon=$mon$day&amp;p=$i\">".($i+1)."</a> "; 
				        elseif ($t1)  {$mrules["{LPAGES}"] .= " ... "; $t1 = 0;}
				        
				        if ($i > ($p-2) && $p > 5 && $p < ($kol_p - 6) && ($i-$p) < 2) $mrules["{LPAGES}"] .= ($i==$p) ? "<span class=\"sp\">".($i+1)."</span> " : "<a href=\"index.php?m=$m&amp;year=$year&amp;mon=$mon$day&amp;p=$i\">".($i+1)."</a> ";
				        elseif ($t2 && $p > 5 && $i > 4 && $p < ($kol_p - 6) && $i > ($kol_p - 5))  {$mrules["{LPAGES}"] .= " ... "; $t2 = 0;}				        
				    
				        
                        if ($i > ($kol_p - 5) || ($p - $i) < 2 && $p > ($kol_p - 7)) $mrules["{LPAGES}"] .= ($i==$p) ? "<span class=\"sp\">".($i+1)."</span> " : "<a href=\"index.php?m=$m&amp;year=$year&amp;mon=$mon$day&amp;p=$i\">".($i+1)."</a> ";
                    }
                    else $mrules["{LPAGES}"] .= ($i==$p) ? "<span class=\"sp\">".($i+1)."</span> " : "<a href=\"index.php?m=$m&amp;year=$year&amp;mon=$mon$day&amp;p=$i\">".($i+1)."</a> ";
    	        }
    	    
			    if ($kol_news - $kol_news_on_p  * ($p + 1) > 0) $mrules["{NEXT}"] = "<a href=\"index.php?m=$m&amp;year=$year&amp;mon=$mon$day&amp;p=".($p+1)."\">".$LANG['snext']."</a>";
