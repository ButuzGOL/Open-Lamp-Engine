...
// Файл: start.php
//-----------------------------------------------------------------------------/
// Назначение: Модуль вывода главной страницы
//=============================================================================/
*/
	...
	$result = $o_news->getS($p, $kol_news_on_p, 0, 0, 0, true, $CONFIG['news_sort'], $CONFIG['news_msort']);
 	if (!$result) $o_other->showMess($LANG['start'], $LANG['news_egets'], "index.php", $LANG['redir_main']. ":)", TEMPLATES_DIR); 
	else {      
	   	$kol_news = $o_news->kolNewsS(0);
	   	$banners_top    = "";
	    $banners_middle = "";
	    $banners_buttom = "";
	    $result1 = $o_banners->getS();
	    if ($result1) {
	   	...
	   	foreach ($result as $n) {
			...
			if ($n[10]) {
				...
				if (!$o_vote->getKolRateOnIp($n[0], IP) && $USER_RIGHTS['allow_rating']) {
					$mrules["{RATE}"] = "<ul class=\"unit-rating\">
	...
	if ($result) {
	    $kol_p = ceil($kol_news / $kol_news_on_p);		if ($kol_p > 1) {
			...
			if ($p > 0) $mrules["{PRIV}"] = "<a href=\"index.php?p=".($p-1)."\">".$LANG['spriv']."</a>";
			...
			for ($i = 0, $t1 = 1, $t2 = 1; $i < $kol_p; $i++) {
				if ($kol_p > 13) {   
				    ...
				    if ($i < 4 || ($i-$p) < 2 && $p < 6) $mrules["{LPAGES}"] .= ($i==$p) ? "<span class=\"sp\">".($i+1)."</span> " : "<a href=\"index.php?p=$i\">".($i+1)."</a> "; 
				    elseif ($t1)  {$mrules["{LPAGES}"] .= " ... "; $t1 = 0;}
				        
				    if ($i > ($p-2) && $p > 5 && $p < ($kol_p - 6) && ($i-$p) < 2) $mrules["{LPAGES}"] .= ($i==$p) ? "<span class=\"sp\">".($i+1)."</span> " : "<a href=\"index.php?p=$i\">".($i+1)."</a> ";
				    elseif ($t2 && $p > 5 && $i > 4 && $p < ($kol_p - 6) && $i > ($kol_p - 5))  {$mrules["{LPAGES}"] .= " ... "; $t2 = 0;}				        
				    
				        
                    if ($i > ($kol_p - 5) || ($p - $i) < 2 && $p > ($kol_p - 7)) $mrules["{LPAGES}"] .= ($i==$p) ? "<span class=\"sp\">".($i+1)."</span> " : "<a href=\"index.php?p=$i\">".($i+1)."</a> ";
                }
                else $mrules["{LPAGES}"] .= ($i==$p) ? "<span class=\"sp\">".($i+1)."</span> " : "<a href=\"index.php?p=$i\">".($i+1)."</a> ";
    	    }
    	    
			if ($kol_news - $kol_news_on_p  * ($p + 1) > 0) $mrules["{NEXT}"] = "<a href=\"index.php?p=".($p+1)."\">".$LANG['snext']."</a>";
